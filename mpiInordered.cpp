...
void inorder(Node* p, int a, int b, vector<vector<Node*>> &Cube, vector<vector<Node*>> &newCube, bool auxflag, vector<vector<int>> &toSendCube)
{
  if(p->left) inorder(p->left, a, b, Cube, newCube, auxflag, toSendCube);
    ...
    if(checkFlag){
      ...
      if(!Find(&Cube[aux_cord_vector[0]][aux_cord_vector[1]], aux_cord_vector[2])){ // death cell in this position
        ...
        if(aux_live_cells>=2 and aux_live_cells<=3){// cell lives
          ...
          if(auxflag){ // meter newCube todo no toSendCube
            buildToSendCube(newCube[aux_cord_vector[0]][aux_cord_vector[1]], toSendCube, aux_cord_vector[0], aux_cord_vector[1]);
          }else{
            if(aux_cord_vector[0] == 1){ // meter apenas 2 linhas do Cubo
              buildToSendCube(newCube[aux_cord_vector[0]][aux_cord_vector[1]], toSendCube, 0, aux_cord_vector[1]);
            }
            if(aux_cord_vector[0] == x_size-2){ // meter apenas 2 linhas do Cubo
            buildToSendCube(newCube[aux_cord_vector[0]][aux_cord_vector[1]], toSendCube, 1, aux_cord_vector[1]);
            }
          }
        }
      }
  ...
  if(p->right) inorder(p->right, a, b, Cube, newCube, auxflag, toSendCube);
  else return;
}
    ...
    for(gen=0;gen<number_gen; ++gen){
      ...
      for(int a=0; a<x_size; ++a){
        for(int b=0; b<cube_size; ++b){
          ...
          if(gen==(number_gen-1)){
            if (z_tree != NULL) inorder(z_tree, a, b, Cubinho, newCube, true, toSendCube); //Iterate in binary tree
          }else{
            if (z_tree != NULL) inorder(z_tree, a, b, Cubinho, newCube, false, toSendCube); //Iterate in binary tree
          }
        }
      }
